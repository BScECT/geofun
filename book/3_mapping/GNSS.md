...
Except for the Introduction, this chapter is reproduced in its entirety from {cite}`Tiberius_2022` (this book is published under a CC BY-NC-SA license). Explicit references to other chapters and sections of this book have been added. The Introduction is adapted from Chapter 12 of {cite}`Tiberius_2022`, with minor reorganization of the material and the omission of one subsection.
...
according to the market report by the European GNSS Agency (GSA), now the European Union Agency for the Space Programme (EUSPA) {cite:p}`bib:GSAreport:report`.
...
the reader is therefore referred to, for instance, the textbooks {cite}`bib:GNSShandbook:book` and {cite}`bib:PNT21:book`.
...
align: center
---
GPS block IIF satellite, built by Boeing. These GPS satellites, 12 in total, have been launched between 2010 and 2016. They have a design lifetime of 12 years. The full GPS constellation nominally consists of 24 satellites. Image courtesy of Boeing~{cite
````
...
The clock error is addressed in \citet[Section~21.2.1]{Tiberius_2022}.
...
Linking to the exposition on measuring distances in \citet[Chapter~20]{Tiberius_2022}, the pseudorange measurement corresponds to
`pulse-based'-ranging, and the carrier phase measurement obviously to `phase-based'-ranging, see \citet[Section~20.1]{Tiberius_2022} on the principles of ranging,
though one should note that GPS is about *one-way* ranging (rather than two-way ranging, as in \citet[Chapter~20]{Tiberius_2022}).
...
with respect to the satellite (along the line of sight), see also \citet[Section~20.2]{Tiberius_2022}. The Doppler frequency, multiplied by the wavelength,
...
namely RINEX, see \citet[Appendix~F]{Tiberius_2022}.
...
align: center
---
GPS positioning --- in three dimensions --- is based on measuring pseudoranges to at least four satellites, of which the positions are known. Visualization by Axel Smits~{cite
````
...
see \citet[Section~8.4]{Tiberius_2022}. The linearized model of observation equations reads

$$
\begin{equation}
\underbrace{
\left( \begin{array}{c} \Delta \underline{p}_{r}^{1} 
\Delta \underline{p}_{r}^{2} 
\vdots 
\Delta \underline{p}_{r}^{m} \end{array} \right)
}_{\Delta \underline{y}} =
\underbrace{
...
\underbrace{
\left( \begin{array}{c} \Delta x_{r} 
\Delta y_{r} 
\Delta z_{r} 
b_r \end{array} \right)
}_{\Delta x} +
\underbrace{
\left( \begin{array}{c} \underline{e}_{r}^{1} 
\underline{e}_{r}^{2} 
\vdots 
\underline{e}_{r}^{m} \end{array} \right)
}_{\underline{e}}

\end{equation}
$$

where we assume to have $m$ satellites in view. \citet[Section~9.3]{Tiberius_2022} presents the linearization of a
...
from the receiver `r', pointing to the satellite `s', cf.\ (\citet[Equation~9.5]{Tiberius_2022}).
...
which minimizes the uncertainty of the solution (see \citet[Chapter~8]{Tiberius_2022}.
Then one can also obtain the variance matrix of the parameter estimators, through (\citet[Equation~8.7]{Tiberius_2022}), and analyse the precision of the position coordinates.
...
solutions is NMEA, see \citet[Appendix~E]{Tiberius_2022}.
...
East, North and Up, see \citet[Section~29.4]{Tiberius_2022}.

Table~{ref}`tab:spppos` presents the resulting empirical mean, standard deviation (std) and the root mean square
(rms), which is the square root of the MSE, see \citet[Chapter~6]{Tiberius_2022},
...
align: center
---
Example of network of permanent GPS tracking stations, of a commercial network RTK service provider in the Netherlands, Belgium and Luxemburg. Image obtained with permission from [06-GPS](https://www.06-gps.nl) {cite
````
...
align: center
---
Excavator in the process of constructing a motorway embankment. RTK-GPS provides accurate real-time position information to guide this machine (note the two GPS-antennas on the back of the engine). Image courtesy of [Heijmans](https://www.heijmans.nl/) {cite
````

Many high-end GPS receivers have RTK functionality built-in, but it can also be performed with professional software,
or even with open source software such as the RTKLIB program package~{cite}`bib:rtklib`.
...
NTRIP has been developed by the German Federal Agency for Cartography and Geodesy (BKG)~{cite}`bib:BKG`.
...
For a more in-depth coverage, the reader is referred to~{cite}`bib:GNSShandbook:book`.
...
(for which an adequate solution is provided by the LAMBDA-method~{cite}`bib:GNSShandbook:book`).
...
align: center
---
Example of a centimeter-accurate 3D Digital Terrain Model (DTM) resulting from Carrier Phase (CP) Real-Time Kinematic (RTK) positioning. The DTM is presented in the national RD-NAP reference system (see \citet[Chapter~35]{Tiberius_2022
````
...
on GPS and GNSS positioning, navigation and timing, the reader is referred to {cite}`bib:GNSShandbook:book` and {cite}`bib:PNT21:book`. These textbooks also cover a wide
range of applications.

The first source of information on GPS, as well as the point of contact is the [Navigation Center of the US Coast Guard](http://www.navcen.uscg.gov/)~{cite}`bib:USCG`.
Official U.S. government information about GPS is available through [GPS.gov](https://www.gps.gov/)~{cite}`bib:GPS`.

The first source of information on Galileo and point of contact is the European Union Agency for the Space Programme ([EUSPA](https://www.euspa.europa.eu/))~{cite}`bib:EUSPA`.
...
and providing GNSS data and products for high(est)-precision applications {cite}`bib:IGS`.
